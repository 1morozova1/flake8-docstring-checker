...
def get_file_content(filepath: Path) -> str:
    '''Получает контент проверяемого файла.

    Args:
        filepath: Путь к файлу.

    Returns:
        Контент проверяемого файла.
    '''
...
class DocstringsChecker:
    ...
    @_add_error
    def check_docstring_exist(self, cur_data: ObjectInfo) -> ERRORS_STRUCTURE:
    ...
    def multiline_closing_quotes_check(
            self, docs_info: Docstring, cur_data: ObjectInfo
    ) -> ERRORS_STRUCTURE:
    ...
    def blank_line_check(
        ...
        Returns:
            Код, сообщение, строка, колонка ошибки, если ошибка. Иначе ничего.
        '''
        if docs_info.blank_after_short_description or \
                docs_info.short_description.endswith(quotes):
            return None
        return (
